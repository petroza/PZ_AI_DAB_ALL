...
const CHUNK = 8 * 1024 * 1024;  // 8 MB na kus (R2 multipart: části ≥5 MB kromě poslední)
let picked = null;
...
async function jget(u){const r=await fetch(u);if(r.status===401){location.reload();throw new Error(401);}if(!r.ok)throw new Error(r.status);return r.json();}
...
function jobCard(j){
  ...
  if(j.status==="done"){
    ...
    if(o.video) outs+='<button class="dlbtn" data-dlvideo="'+esc(j.id)+'" data-fn="'+esc(j.filename||"")+'" title="Stáhne video a smaže zakázku ze serveru">⬇ Video</button>';
...
// Stáhni video do zařízení a AŽ po úspěšném stažení smaž zakázku ze serveru
// (uvolní R2). Mazání až po dokončení – kdyby spojení spadlo, o soubor nepřijdeš.
async function downloadVideoAndPurge(id, fn, btn){
  const orig = btn ? btn.textContent : "";
  if(btn){ btn.disabled=true; btn.textContent="Stahuji…"; }
  try{
    const r=await fetch(API+"?action=download&id="+encodeURIComponent(id)+"&kind=video");
    if(!r.ok) throw new Error("stažení selhalo ("+r.status+")");
    const blob=await r.blob();
    if(!blob.size) throw new Error("prázdný soubor");
    const base=((fn||"video").replace(/\.[^.]+$/,"")) || "video";
    const a=document.createElement("a");
    a.href=URL.createObjectURL(blob);
    a.download=base+".dubbed.mp4";
    document.body.appendChild(a); a.click(); a.remove();
    setTimeout(()=>URL.revokeObjectURL(a.href), 15000);
    // teprve teď – po kompletním stažení – smaž ze serveru
    const fd=new FormData(); fd.append("action","delete"); fd.append("id",id);
    await fetch(API,{method:"POST",body:fd});
    refresh();
  }catch(e){
    alert("Chyba stažení: "+e.message+"\n(zakázka zůstává na serveru)");
...
$("jobs").addEventListener("click",e=>{
  ...
  const dv=e.target.closest("[data-dlvideo]");
  if(dv){ downloadVideoAndPurge(dv.getAttribute("data-dlvideo"), dv.getAttribute("data-fn"), dv); return; }
